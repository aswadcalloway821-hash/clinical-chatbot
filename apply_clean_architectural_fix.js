...
// 1. حذف الملف الزائد src/routes/api.routes.ts
...
// 2. تحديث ai.service.ts وحذف جميع الـ Fallback الجاهزة أو الثابتة بالكامل
const aiServiceContent = `import { ClinicContext, AvailableSlot } from './booking.service';
...
export class AIService {
  /**
   * 🧠 محرك المحادثة الصافي الاعتماد المطلق على Gemini 3.1 Flash Lite 100% بدون أي Fallback صلب أو ردود ثابتة
   */
  async processPureNLU(
    ...
    // تطهير مصفوفة المحادثة من أي عبارات قديمة مكررة
    ...
    // صياغة المواعيد الشاغرة الحقيقية غير المحجوزة
...
console.log('✅ Updated src/services/ai.service.ts cleanly without any Fallbacks');

// 3. تحديث booking.service.ts لمنع اختيار المواعيد المحجوزة إطلاقاً وتفادي خطأ Supabase RPC Slot already booked
const bookingServiceContent = `import { supabase } from '../config/supabase';
...
fs.writeFileSync('src/services/booking.service.ts', bookingServiceContent, 'utf8');
console.log('✅ Updated src/services/booking.service.ts to dynamically find unbooked slots and prevent RPC overlap errors');
