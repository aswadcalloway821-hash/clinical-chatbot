...
export class BookingService {
  /**
   * 🔑 1. تحديد clinic_id بـ RPC resolve_clinic_context_from_channel
   */
  ...
  /**
   * 🧠 3. إدارة جلسة المريض عبر RPC get_or_create_patient_session
   */
  ...
  /**
   * 🔄 4. تحديث حالة وتفاصيل المريض بـ Supabase
   */
  ...
  /**
   * 📅 5. جلب أقرب الشواغر الحقيقية غير المحجوزة
   */
  ...
  /**
   * 📊 7. المعالجة التفاعلية الصافية بدون إعادة كتابة أي نص من Gemini
   */
  async processTestMessageWithTelemetry(
    ...
    // 1. جلب الشواغر الحقيقية لحقنها في البرومبت
    ...
    // 2. تحليل وتوليد الرد الصافي بالكامل من Gemini 3.1 Flash Lite
    const aiResult = await aiService.processPureNLU(clinicCtx, history, cleanText, nearestSlots);

    // ⚠️ عدم إعادة كتابة النص نهائياً! استخدام رد Gemini الصافي 100%
