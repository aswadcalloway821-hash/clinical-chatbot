...
export class BookingService {
  /**
   * 🔑 1. تحديد clinic_id مرة واحدة بـ RPC: resolve_clinic_context_from_channel
   */
  async resolveClinicId(channelIdentifier: string): Promise<{ clinic_id: string; clinic_name: string }> {
    try {
      ...
      console.warn('⚠️ resolve_clinic_context_from_channel fallback:', err.message);
    }
  ...
  /**
   * 🏛️ 2. استعلام كافة حقائق العيادة والأطباء وgoogle_calendar_id من v_clinic_full_context
   */
  async getClinicContext(clinicId: string): Promise<ClinicContext> {
    try {
      ...
      console.warn('⚠️ getClinicContext view error:', err.message);
      return {
        clinic_id: '22222222-2222-2222-2222-222222222222',
        clinic_name: 'عيادة ابتسامة البصرة',
  ...
  /**
   * 🧠 3. إدارة جلسة المريض عبر RPC get_or_create_patient_session و update_patient_session_state
   */
  ...
  /**
   * 🔄 4. تحديث حالة وتفاصيل المريض عبر RPC update_patient_details و update_patient_session_state
   */
  async updatePatientProfileAndState(
    ...
    try {
      ...
      console.warn('⚠️ updatePatientProfileAndState error:', err.message);
    }
  }

  /**
   * 🔄 تصفير الجلسة للاختبار
   */
  ...
  /**
   * 📋 جلب الحجوزات المفعلة بـ RPC get_patient_active_bookings
   */
  ...
  /**
   * 📅 5. البحث عن أطباء الخدمة ومراجعة Google Calendar الشاغرة (من 4 عصراً إلى 9 مساءً)
   */
  async getNearestAvailableSlot(
    ...
    // مطابقة طبيب الأسنان أو الخدمة المطلوبة
    ...
    // الاستعلام عن الحجوزات الموجودة لمنع تكرار أي حجز
  ...
  async processTestMessageWithTelemetry(
    ...
    if (isConfirmIntent) {
      ...
      const slot1 = await this.getNearestAvailableSlot(activeClinicId, aiResult.extractedDetails?.preferred_doctor, aiResult.extractedDetails?.preferred_service || cleanText, undefined, undefined, 1);
      const slot2 = await this.getNearestAvailableSlot(activeClinicId, aiResult.extractedDetails?.preferred_doctor, aiResult.extractedDetails?.preferred_service || cleanText, undefined, undefined, 2);
