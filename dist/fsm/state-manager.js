...
export class FsmStateManager {
    ...
    static async processMessage(phone, messageText, tenant) {
        ...
        const nluResult = await GeminiService.parseNluIntent(messageText, session.currentState, tenant);
        // 2. Check for Human Handoff trigger (Immediate transfer on Complaint / Anger / Human Request)
        if (nluResult.intent === 'REQUEST_HUMAN' ||
            ...
            case 'CONFIRMATION_PENDING':
                if (nluResult.intent === 'CONFIRM' || messageText.includes('نعم') || messageText.includes('تأكيد') || messageText.includes('اوكي')) {
                    ...
                    if (session.selectedSlot)
                        SlotGenerator.unlockSlot(session.selectedSlot);
