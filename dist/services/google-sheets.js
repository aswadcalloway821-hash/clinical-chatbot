...
export class GoogleSheetsService {
    ...
    static async getTenantConfig(tenantId = 'live_sheet') {
        ...
        const branches = dataRows.map((r, idx) => {
            ...
            if (!bName)
                throw new Error(`[Google Sheets Error] Missing branch name at row ${idx + 2} in 'Clinic_Metadata'.`);
            return {
                id: `b_${idx + 1}`,
                name: bName,
        ...
        const doctors = docDataRows.map((d, idx) => {
            ...
            if (!docName)
                throw new Error(`[Google Sheets Error] Missing doctor name at row ${idx + 2} in 'Doctors_Config'.`);
        ...
        const services = servDataRows.map((s, idx) => {
            ...
            if (!sName)
                throw new Error(`[Google Sheets Error] Missing service name at row ${idx + 2} in 'Services_Config'.`);
            return {
                id: `s_${idx + 1}`,
                name: sName,
